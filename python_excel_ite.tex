...
The stage is set with an overview of a typical spreadsheet based model that most of the students have seen before -- see Figure \ref{fig:stage}. The notebook also includes a screenshot (Figure \ref{fig:xl_model}) of a typical Excel model.
...
Students are given skeleton code as scaffolding and must finish the expressions for computing key intermediate outputs (\code{order\_cost, sales\_revenue, refund\_revenue}) and the final output, \code{profit}. 
